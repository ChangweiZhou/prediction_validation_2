...
function setup_testing_input_output {
  ...
  cp -r ${GRADER_ROOT}/tests/${test_folder}/input/actual.txt ${TEST_OUTPUT_PATH}/input/actual.txt
  cp -r ${GRADER_ROOT}/tests/${test_folder}/input/predicted.txt ${TEST_OUTPUT_PATH}/input/predicted.txt
  cp -r ${GRADER_ROOT}/tests/${test_folder}/input/window.txt ${TEST_OUTPUT_PATH}/input/window.txt
}

function compare_outputs {
  NUM_OUTPUT_FILES_PASSED=0
  OUTPUT_FILENAME=comparison.txt
